...
UndistorterPTAM::UndistorterPTAM(const char* configFileName)
{
	...
	if(valid)
	{
		...
		fx = static_cast<float>(fx * xfactor);
		fy = static_cast<float>(fy * yfactor);
		cx = static_cast<float>((cx + 0.5) * xfactor - 0.5);
		cy = static_cast<float>((cy + 0.5) * yfactor - 0.5);
...
void UndistorterPTAM::undistort(const cv::Mat& image, cv::OutputArray result) const
{
	...
	for(int idx = out_width*out_height-1;idx>=0;idx--)
	{
		...
		else
		{
			...
			data[idx] = static_cast<uchar>( xxyy * src[1+in_width]
