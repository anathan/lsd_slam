	...
	for(int i=0;i<width;i++)
		for(int j=0;j<height;j++)
		{
			...
			if(id >=0 && idepthVar[i + j*width] >= 0)
			{
				...
				uchar rc = static_cast<uchar>( r < 0 ? 0 : (r > 255 ? 255 : r));
				uchar gc = static_cast<uchar>(g < 0 ? 0 : (g > 255 ? 255 : g));
				uchar bc = static_cast<uchar>(b < 0 ? 0 : (b > 255 ? 255 : b));
			...
			else
			{
				...
				for(int dx=-2; dx <=2; dx++)
					for(int dy=-2; dy <=2; dy++)
					{
						...
						{
							float distFac = static_cast<float>((float)(dx*dx + dy*dy)*(0.075*0.075)*0.02);
	...
	for(int i=0;i<width;i++)
		for(int j=0;j<height;j++)
		{
			...
			if(idv > 0)
			{
				...
				res.at<cv::Vec3b>(j, i) = cv::Vec3b(0, static_cast<uchar>(255 - var), static_cast<uchar>(var));
			}
		}
